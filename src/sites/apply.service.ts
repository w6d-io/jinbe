...
function withVersion(cr: SiteCr, version: number): SiteCr {
...
async function applyRecord(record: SiteRecord, actor: Actor, verb: string) {
  ...
  auditSite('apply', site.name, actor, `${verb} version ${version}`, { version, rules: rendered.rules.map((r) => r.id) }, 'applied')
  return { applyId: randomUUID(), version, rules: rendered.rules.map((r) => r.id), site: rendered.siteCr.metadata.name }
}
